...
export class AjusteInventarioService {
    ...
    async create(createDto: CreateAjusteInventarioDto) {
        ...
        createDto.detalle.forEach(item => {
            ...
            if (!item.importe_ml) {
                item.importe_ml = item.cantidad * costo_ml;
            }
            if (!item.importe_me && item.costo_me) {
                item.importe_me = item.cantidad * costo_me;
            }
        ...
        return this.movimientoStockService.create({
            ...
            detalle: createDto.detalle.map(d => ({
                ...d,
                // Asegurar valores por defecto
                costo_ml: d.costo_ml || 0,
                precio_ml: d.precio_ml || 0,
                porcentaje_iva: 0,
                porcentaje_descuento: 0,
                descuento_ml: 0,
                importe_ml: d.importe_ml || (d.cantidad * (d.costo_ml || 0))
            }))
        });
    }
